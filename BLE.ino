...
BLEFloatCharacteristic Press_Characteristic("1e55006-e229-4d47-ac2d-59871700c944", BLERead | BLENotify);
BLEUnsignedLongCharacteristic Config_Characteristic("1e55007-e229-4d47-ac2d-59871700c944", BLERead | BLEWrite | BLENotify);
...
void setupBLE() {
  ...
  blePeripheral.addAttribute(Press_Characteristic);
  ...
  Press_Characteristic.setValue(0);
...
void maybePublishTempHumPress(float temp, float hum, float press) {
  ...
  if (config_data_.ble_data_mode == BLE_SEND_ALWAYS ||
      abs(press - last_sent_aq_data_.press) > 10) {
    Press_Characteristic.setValue(press);
    last_sent_aq_data_.press = press;
  }
}
